  ...
  const handleSortEnd = useCallback((dragIndex: number, hoverIndex: number) => {
    setState(({ vttCues }) => ({
      vttCues: update(vttCues, {
        $splice: [
          [dragIndex, 1],
          [hoverIndex, 0, vttCues[dragIndex] as VTTCue]
        ]
      })
    }))
  }, [setState])
        ...
        <div className={css({ width: "640px", overflowY: "scroll", border: "1px solid #222222" })} >
          ...
          <ul className={css({ padding: 0 })}>
            {state.vttCues.length !== 0 ? state.vttCues.map((cue, index) => {
              return (<TrackListItem index={index} cue={cue} duration={state.duration} onSortEnd={handleSortEnd} onDelete={() => deleteTrack(cue.id)} onChangeTrackText={(e) => handleChangeTrackText(index, e.currentTarget.value)} onChangeStartTime={(e) => handleChangeStartTime(index, Number.parseInt(e.currentTarget.value))} onChangeEndTime={(e) => handleChangeEndTime(index, Number.parseInt(e.currentTarget.value))} />)
